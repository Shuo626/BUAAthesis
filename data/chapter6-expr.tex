...
如上图\ref{expr_local}所示，可以看到，部署到本地的模型在CPU和GPU上分别执行100次推理时,GPU只需要1.28s而CPU需要5.49s。
...
如上图\ref{expr_android}所示，虽然在Android设备上使用了GPU，但是反而比CPU慢，因为TVM没有针对该GPU的优化，所以导致了执行速度下降明显。
